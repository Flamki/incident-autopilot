...
import { ArrowRight, Database, MoreVertical, Plus, RefreshCw, Search } from "lucide-react";
import { Button } from "@/src/components/ui/Button";
import { Badge } from "@/src/components/ui/Badge";
import { cn } from "@/src/lib/utils";
import { api, Repository } from "@/src/lib/api";
      ...
      <div className="flex items-center justify-between mt-sp-1 px-1">
        ...
        <div className="flex items-center gap-sp-3">
          <div className="relative group">
            ...
            <input type="text" placeholder="SEARCH_REPOS..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-bg-card border border-border h-9 pl-9 pr-4 text-[10px] font-mono font-bold uppercase focus:outline-none focus:border-ink w-64" />
          </div>
          <Button variant="outline" size="sm" onClick={loadRepos}>
            <RefreshCw className="mr-2 h-3 w-3" /> Sync
          </Button>
          <Button
            className="btn-neon h-9 px-4 text-[10px] font-bold uppercase tracking-tighter"
            onClick={async () => {
              const now = Date.now();
              await api.createRepo({
                gitlab_project_id: now,
                project_path: `org/new-service-${String(now).slice(-4)}`,
                branch: "main",
                type: "Service",
              });
              loadRepos();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-sp-6">
        {filteredRepos.length > 0 ? (
          filteredRepos.map((repo, i) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-sp-4 border-t border-border border-dashed">
                <Button variant="ghost" size="sm" className="h-7 px-2 text-[9px]" onClick={async () => {
                  await api.testRepo(repo.id);
                }}>
                  Test Connection <ArrowRight className="ml-1 h-3 w-3" />
                </Button>
                <Button variant="outline" size="sm" className="h-7 px-2 text-[9px]" onClick={async () => {
          ...
          <div className="col-span-full py-sp-20 text-center border-2 border-dashed border-border bg-bg-card">
            <p className="font-mono text-[11px] font-bold text-text-muted uppercase tracking-widest">No repositories match query.</p>
          </div>
        )}
      </div>
    </div>
